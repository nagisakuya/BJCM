...
impl TableState{
    pub fn show_deck(&self, ui: &mut Ui, config: &Config) {
        ...
        for i in 0..10 {
            ...
            _count += ((config.rule.NUMBER_OF_DECK * if i != 9 { 4 } else { 16 }) - self.deck[i])
                as f64
                * GRIFFIN_ULTIMATE[i];
        }
        _count = _count * GRIFFIN_ULTIMATE_MULTIPLER / config.rule.NUMBER_OF_DECK as f64;
        /*ui.label(RichText::new(format!(
            "{}={:4>+1.3}%",
    ...
    pub fn draw_table(&mut self, ui: &mut Ui) {
        ...
        //draw dealer
        {
            ...
            let dealer_root = pos2(available_rect.left() + available_rect.width() / 2.0 - Self::CARD_SIZE.x/2.0 - 25.0, available_rect.top() + TOP_MARGIN);
